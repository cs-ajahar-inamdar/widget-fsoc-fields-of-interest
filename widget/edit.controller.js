...
(function () {
    angular
        .module('cybersponse')
        .controller('editFsocFieldsOfInterest100Ctrl', editFsocFieldsOfInterest100Ctrl);

    editFsocFieldsOfInterest100Ctrl.$inject = ['$scope', '$uibModalInstance', 'config', '_', '$state', 'Entity', 'widget', 'ViewTemplateService', 'CommonUtils'];

    function editFsocFieldsOfInterest100Ctrl($scope, $uibModalInstance, config, _, $state, Entity,  widget, ViewTemplateService, CommonUtils) {
        $scope.cancel = cancel;
        $scope.save = save;
        $scope.widget = widget;
        $scope.config = config;
